<template>
	<ml-dialog v-model="visible" title="自定义按钮设置" width="760px" append-to-body>
		...
		<el-row v-loading="loading" :gutter="20" class="button-setting-content" element-loading-text="加载中...">
						...
						>
							...
							>
								...
								>
									...
									<span v-if="button.isNative">(内置)</span>
								</div>
								<div v-if="!button.isNative" class="button-list-item-icon">
			...
			<el-col :span="16">
				...
				<el-form v-else class="button-editor">
					<el-row :gutter="16">
						<el-col :span="12">
								<el-form-item label="按钮名称" class="has-required">
								<el-input v-model="currentButton.name" placeholder="请输入按钮名称" clearable />
							</el-form-item>
						</el-col>
						<el-col :span="12">
						...
						<el-divider v-if="!currentButton.isNative" />
						<el-col v-if="!currentButton.isNative" :span="24">
							<el-form-item label="叠加自定义权限" class="custom-right-form-item">
								<div class="custom-right-box">
									...
									<el-tooltip content="取反后，用户不包含选定条件菜单才会显示。" placement="top">
										<el-checkbox v-model="currentButton.reversalCustomCode" label="取反" />
									</el-tooltip>
								</div>
							</el-form-item>
						</el-col>
...
import { getSystemConstants } from "@/api/system-manager";
...
const isCustomActionButton = (button) => !button?.isNative && (Number(button?.action) === 4 || button?.action === "custom");
...
const loadCustomRightList = async () => {
	try {
		const res = await getSystemConstants("customRight");
		customRightList.value = res?.data || [];
	} catch (error) {
		console.error("load custom right list error", error);
		customRightList.value = [];
	}
};
...
const mergeNativeTopButtons = (buttons) => {
	...
	if (!ordered.length) return nativeTopButtonDefaults.map((button) => ({ ...button }));
...
const openDialog = async () => {
	...
	try {
		...
		if (customButton?.config) {
			...
			topButtons.value = mergeNativeTopButtons(config.pcTop);
			columnButtons.value = Array.isArray(config.pcColumn)
				? config.pcColumn.map(normalizeButton).filter(isCustomActionButton)
				: [];
		} else {
			topButtons.value = nativeTopButtonDefaults.map((button) => ({ ...button }));
		}
		currentButton.value = activeButtonList.value[0] || null;
	} catch (error) {
...
const deleteButton = async (button) => {
	...
	if (index === -1) return;
	try {
		await ElMessageBox.confirm(`确定要删除【${getButtonName(button)}】吗？`, "提示", {
