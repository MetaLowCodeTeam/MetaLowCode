<template>
	<div v-if="optionModel.barChartConfig">
		<el-form-item label="图表标题">
			<el-input v-model="optionModel.barChartConfig.title" />
		</el-form-item>
		<el-form-item label="隐藏标题">
			<el-switch v-model="optionModel.barChartConfig.hideTitle" />
		</el-form-item>
		<el-form-item label="宽度">
			<el-input v-model="optionModel.barChartConfig.width" placeholder="100%" />
		</el-form-item>
		<el-form-item label="高度">
			<el-input-number v-model="optionModel.barChartConfig.height" :min="120" :max="1200" style="width: 100%" />
		</el-form-item>
		<el-form-item label="显示图例">
			<el-switch v-model="optionModel.barChartConfig.showLegend" />
		</el-form-item>
		<el-form-item label="显示数值">
			<el-switch v-model="optionModel.barChartConfig.showLabel" />
		</el-form-item>
		<el-form-item label="绑定数据模型">
			<el-select v-model="optionModel.barChartConfig.bindModelCode" placeholder="请选择数据模型" filterable clearable>
		...
		<el-drawer v-model="dimensionDialogVisible" title="维度指标设置" size="460">
			<div class="drawer-main">
				...
				<div class="item-box" :style="{ height: itemBoxHeight }">
					...
					<el-empty v-if="availableFields.length === 0" description="请先将柱状图拖入已绑定元数据的循环容器" />
...
export default {
	...
	computed: {
		...
		availableFields() {
			const bindCode = this.optionModel.barChartConfig?.bindModelCode
			const models = this.getMetaModels(this.getMetaFields())
	...
	methods: {
		initDimensional() {
			if (this.optionModel.setDimensional && !this.optionModel.barChartConfig.setDimensional) {
				this.optionModel.barChartConfig.setDimensional = cloneDeep(this.optionModel.setDimensional)
			}
			if (this.optionModel.setDimensional) {
				delete this.optionModel.setDimensional
			}
			if (!this.optionModel.barChartConfig.setDimensional) {
				this.optionModel.barChartConfig.setDimensional = { dimension: [], metrics: [] }
			}
			this.dimension = this.optionModel.barChartConfig.setDimensional.dimension || []
			this.metrics = this.optionModel.barChartConfig.setDimensional.metrics || []
		...
		syncDimensional(target) {
			...
			if (target === 'dimension') {
				...
				this.optionModel.barChartConfig.setDimensional.dimension = this.dimension
			} else if (target === 'metrics') {
				this.metrics = cloneDeep(this.draggableMetrics.length ? this.draggableMetrics : this.metrics)
				this.optionModel.barChartConfig.setDimensional.metrics = this.metrics
			}
			this.draggableDimension = cloneDeep(this.optionModel.barChartConfig.setDimensional.dimension)
			this.draggableMetrics = cloneDeep(this.optionModel.barChartConfig.setDimensional.metrics)
		},
		commitDimensional() {
			if (!this.optionModel.barChartConfig.setDimensional) {
				this.optionModel.barChartConfig.setDimensional = { dimension: [], metrics: [] }
		...
		openDimensionDialog() {
			if (!this.optionModel.barChartConfig?.bindModelCode) {
