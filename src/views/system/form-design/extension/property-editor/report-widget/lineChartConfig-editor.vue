<template>
	<div v-if="optionModel.lineChartConfig">
		...
		<el-form-item label="绑定数据模型">
			<el-select v-model="optionModel.lineChartConfig.bindModelCode" placeholder="请选择数据模型" filterable clearable>
...
export default {
	...
	data() {
		return {
			...
			draggableDimension: [],
			draggableMetrics: [],
			dimensionGroupName: 'line-chart-dimension-fields',
	...
	computed: {
		...
		availableFields() {
			const bindCode = this.optionModel.lineChartConfig?.bindModelCode
			const models = this.getMetaModels(this.getMetaFields())
	...
	methods: {
		...
		openDimensionDialog() {
			if (!this.optionModel.lineChartConfig?.bindModelCode) {
