...
<template>
	<div class="ml-water-mark" ref="mlWaterMark">
		<slot></slot>
	</div>
</template>
	...
	export default {
		...
		methods: {
			create(){
				...
				this.$refs.mlWaterMark.appendChild(watermark)
			},
			clear(){
				var wmDom = this.$refs.mlWaterMark.querySelector('.watermark')
				wmDom && wmDom.remove()
			}
		}
	}
...
<style scoped>
	.ml-water-mark {position: relative;display: inherit;width: 100%;height: 100%;}
</style>
